...
export const connectionService = {
  ...
  async getById(id: string): Promise<ConnectionConfig | null> {
    const result = await db.getFirstAsync('SELECT * FROM connections WHERE id = ?', [id])
  ...
  async create(connection: ConnectionConfig): Promise<void> {
    await db.runAsync(
      `INSERT INTO connections (id, name, host, port, username, auth_method, private_key, key_id, password, "group", profile) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        connection.id,
        connection.name,
  ...
  async update(connection: ConnectionConfig): Promise<void> {
    await db.runAsync(
      `UPDATE connections SET name = ?, host = ?, port = ?, username = ?, auth_method = ?, private_key = ?, key_id = ?, password = ?, "group" = ?, profile = ?
       WHERE id = ?`,
      [
        connection.name,
        connection.host,
