...
const ShellHistoryHookScriptBash = `[[ -n "${__FREESSH_BASH_HOOK_INSTALLED:-}" ]] || { __freessh_emit_history(){ local line histno cmd; line="$(history 1 2>/dev/null)"; histno="$(printf '%s\n' "$line" | awk '{print $1}')"; cmd="$(printf '%s\n' "$line" | sed 's/^[[:space:]]*[0-9][0-9]*[[:space:]]*//')"; if [ -n "$histno" ] && [ "$histno" != "${__FREESSH_LAST_HISTNO:-}" ]; then __FREESSH_LAST_HISTNO="$histno"; printf '\033]1337;freessh-history=%s\a' "$cmd"; fi; }; case ";${PROMPT_COMMAND:-};" in *";__freessh_emit_history;"*) ;; *) PROMPT_COMMAND="__freessh_emit_history${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;; esac; __FREESSH_BASH_HOOK_INSTALLED=1; }`
const ShellHistoryHookScriptZsh = `[[ -n "${__FREESSH_ZSH_HOOK_INSTALLED:-}" ]] || { autoload -Uz add-zsh-hook 2>/dev/null || true; __freessh_precmd(){ local histno cmd; histno="$HISTCMD"; cmd="$(fc -ln -1 2>/dev/null)"; if [ -n "$histno" ] && [ "$histno" != "${__FREESSH_LAST_HISTNO:-}" ]; then __FREESSH_LAST_HISTNO="$histno"; printf '\033]1337;freessh-history=%s\a' "$cmd"; fi; }; typeset -pm precmd_functions 2>/dev/null | grep -q "__freessh_precmd" || add-zsh-hook precmd __freessh_precmd 2>/dev/null || true; __FREESSH_ZSH_HOOK_INSTALLED=1; }`
...
func SelectHookScript(shellName, osType string) string {
	...
	case strings.Contains(shell, "zsh"):
		return ShellHistoryHookScriptZsh
	case strings.Contains(shell, "bash"):
		return ShellHistoryHookScriptBash
	default:
		return ""
	}
}
